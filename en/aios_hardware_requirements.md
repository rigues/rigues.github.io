...
parent: '# Overview of the AIOS Platform'
nav_order: 2
description: 'Descrição dos requisitos de hardware da plataforma AIOS, incluindo câmeras,
  AIBox e parâmetros relacionados. '
---
Aqui está a tradução do texto para o inglês, mantendo a formatação Markdown:
...
* USB 2.0 or USB 3.0 cameras directly connected to the AIBox.
...
| Resolution | 480p |
| Framerate | 5 FPS |
| Encoding^[1]^ | **H.264** or **H.265**. We recommend a constant bitrate (**CBR**, _Constant Bitrate_) for applications requiring number and letter recognition (such as **OCR** and **ALPR**) or faces, and variable bitrate (**VBR**, _Variable Bitrate_) for other applications. |
...
Note that in monitoring systems, the concept of "pixels per meter" (**PPM**) is more important than an absolute measure of resolution in Megapixels (**MP**). It indicates how many image pixels are needed to cover an object 1 meter long in the observed scene. The higher the PPM, the more details the camera can capture, allowing for the identification of features such as faces, car license plates, or text.
...
![BMW electric car seen from behind stopped at an intersection, with visible rear license plate and red traffic light ahead on a tree-lined avenue.](./images/ppm_example.jpg "Illustration of the PPM concept")

The image above illustrates the PPM concept well. Both the vehicle's license plate and the traffic lights in the background have the same width, about 30 cm. However, since the license plate is closer to the camera, it occupies a larger area of the image (314 pixels for the license plate, compared to 83 pixels for the traffic lights). This means that with the same camera, the license plate's PPM is higher than that of the traffic lights.
...
![Illustration of a wall-mounted security camera pointing at a stylized standing character, with a dashed line indicating the line of sight and the vertical angle of incidence θ between the horizontal and the camera.](./images/vertical_angle_incidence.png "Representation of the vertical angle of incidence")
...
![Illustration of a security camera seen from above, with two people positioned on the left and right, and dashed lines indicating the line of sight for each, forming horizontal angles A and B in red.](./images/vertical_angle_incidence.png "Representation of the horizontal angle of incidence")
...
These parameters do not apply to the Scheduler, PLC, Polygon Detection, Detection Filter, Virtual Line, Virtual Line with Direction, Moni, Email Notification, Kanban Notification, Zone Permanence, Output Pin, and WhatsApp components.

## AIBox
In AIOS, all image and sensor data processing is performed on the AIBox, a powerful hardware device designed to locally execute AI models (Edge AI) with high performance and low power consumption.

![Compact black industrial computer on an office desk, with finned heat dissipation casing and various connection ports on the rear, including Ethernet, USB, and green I/O connectors.](./images/aibox.png "An AIBox")
...
The AIBox has an IP (_Ingress Protection_) 40 certification against object ingress. As a precaution, contact our team to discuss providing adequate protection if installation is required in environments where it may be exposed to moisture condensation, fine particles, or risk of immersion in water or other liquids.

## Connectivity
For proper functioning, the AIBox needs to connect to external servers. If internet access is controlled by a Firewall, ensure that access to the addresses, protocols, and ports included in the table below is enabled.
...
_**[1]** TailScale is a VPN service used for remote access by the dtLabs technical support team. Therefore, it is essential that access to this VPN is enabled._
